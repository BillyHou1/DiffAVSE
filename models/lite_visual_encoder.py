...
class LiteVisualEncoderA(nn.Module):
    ...
    def forward(self, video, T_audio):
        ...
        visual_raw = F.interpolate(
            temporal_features, size=T_audio, mode="linear", align_corners=False
        )
...
if __name__ == "__main__":
    cfg = {"visual_cfg": {"encoder_type": "A", "freeze_visual_encoder": True}}

    batch_size = 2
    T_video = 75   # 3s video @ 25fps
    T_audio = 480  # example STFT frames
    ...
    print(f"EncoderA output shape: {visual_raw.shape}")

    print("\nEncoderA parameter freeze check:")
    trainable_params, frozen_params = [], []
    for name, param in encoder_a.named_parameters():
        (trainable_params if param.requires_grad else frozen_params).append(name)
    ...
    trainable_count = sum(p.numel() for p in encoder_a.parameters() if p.requires_grad)
    frozen_count = sum(p.numel() for p in encoder_a.parameters() if not p.requires_grad)

    print("\nEncoderA parameter statistics:")
    ...
    print(f"EncoderB total parameters: {total_params_b:,}")
